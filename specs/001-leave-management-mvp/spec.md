...
**Created**: 2026-07-15
**Last Refactored**: 2026-08-05 (Costa Rica business time clarified)
...
**Governance**: This specification is subordinate to `.specify/memory/constitution.md` v7.0.0. The constitution remains authoritative for Clean Architecture, ASP.NET Core MVC, Razor Views, Bootstrap, ASP.NET Core Identity, persistence, security, testing, observability, and engineering governance. This specification defines business behavior and observable outcomes. It does not prescribe controllers, repositories, database tables, HTTP routes, framework classes, or code structure.
...
**Constitution Alignment Note**: The two behaviors that previously conflicted with Constitution v3.0.0 — Approver deactivation of an `Approved` request (former invariants 8–9) and removal of per-user time zones (former invariant 4) — are **reconciled by the Constitution v4.0.0 amendment (2026-07-16)**. The single system business date is now explicitly the Costa Rica date (`America/Costa_Rica`); this does not reintroduce per-user time-zone behavior and timestamps remain stored in UTC. Constitution v7.0.0 remains the current authority. **RESOLVED — User cancellation of a Pending request is OUT OF MVP SCOPE.** No unresolved Constitution conflict remains.
---
...
- Q: Which application roles exist? → A: **`User`, `Approver`, and `HR`.** No teams, managers, delegates, escalation. Any active Approver with `canResolveRequests=true` may resolve any eligible request except their own. HR has organization-wide read access to requests, calendar, balances, audit, and approver-capability management (activate/deactivate `canResolveRequests` for existing Approvers with reason, confirmation, row version, audit) per current Constitution §4.3.
...
- Q: May a User independently cancel their own Pending request? → A: **RESOLVED — User cancellation of a Pending request is OUT OF MVP SCOPE.** No User cancellation route, action, contract, requirement, or transition is part of the MVP.

### Session 2026-08-05 (Costa Rica business time)
- Q: Which date determines “today”, next-day validation, calendar highlighting, accrual, timeout, and pre-start deactivation? → A: **The single system business date in `America/Costa_Rica` (UTC−06:00).** There is no per-user time zone.
- Q: How are timestamps persisted and displayed? → A: **Persist in UTC; convert to Costa Rica time for every user-facing date-time display and label the zone where time-of-day is shown.**
...
1. **AC-057 — Access the basic vacation calendar**
   ...
   **Given** an authenticated User, **When** the User opens the vacation calendar, **Then** the system presents a basic visual calendar of the authorized vacation periods. *(Whether it shows only the User's own requests or broader anonymized availability is resolved — see OQ-005.)*
...
*Retired identifiers (behavior removed with the prior model): FR-008 is reserved because OQ-001 is resolved out of MVP scope and is intentionally not reused. FR-009 (HR read-only visibility) is **active** under current Constitution §4.3. AUTHZ-004 (HR read-only restriction) is **revised** — HR has authorized read access but MUST NOT approve, reject, deactivate, modify balances, or assign/remove roles.*
...
- **FR-025 — Expose Newly Created Pending Requests to Eligible Approvers** *(Event-Driven)* — When a valid vacation-request creation transaction completes, the system shall make the resulting Pending request available exactly once in the approval queue of every active eligible Approver other than its owner, without requiring manual synchronization or application restart.
  *Boundary note (2026-07-29, review finding F7)*: FR-023–FR-025 define server-derived, observable business outcomes that happen to surface in views. They remain in this domain specification under the recorded boundary rule: **spec 001 owns observable business behavior and server-derived values; spec `002-role-based-frontend-views` owns presentation, navigation, layout, and view composition.** The rendering of these outcomes is specified in spec 002; the outcomes themselves are normative here.
...
#### HR Capabilities *(Current Constitution §4.3)*
...
- **FR-017 — HR Organizational Calendar** *(State-Driven)* — While authenticated as an active HR identity, the system shall provide a dedicated read-only month-view calendar at `/rrhh/calendario` showing all vacation requests organization-wide, including all departments, with requester names and request status visible as authorized HR data. HR MUST NOT access organizational calendar behavior through `/calendario`.
...
- **BR-003 — Evaluate Dates in Costa Rica Business Time** *(Ubiquitous)* — The system shall evaluate all date-dependent rules against the single system business date in `America/Costa_Rica` (UTC−06:00), shall not apply per-user time-zone behavior, shall persist timestamps in UTC, and shall convert timestamps to Costa Rica time for user-facing presentation. *(Aligned with Constitution v7.0.0 invariant 4.)*
...
#### Request Lifecycle
- **BR-005 — Initialize a Request as Pending** *(Event-Driven)* — When a request is successfully created, the system shall assign `Pending` as its initial state.
...
- **AUTHZ-002 — Authorize Eligible Active Approver Globally** *(Ubiquitous)* — The system shall authorize any active Approver with `canResolveRequests=true` to access approval queues/details and approve, reject, or deactivate any eligible request in the system, without team, hierarchy, department, or organizational scope.
...
- **AUTHZ-006 — Evaluate Authorization at Operation Time** *(Ubiquitous)* — The system shall determine authorization from authoritative current identity, role, active status, `canResolveRequests` where an Approver approval view or action is involved, ownership, and request state rather than from client-supplied or previously displayed values.
- **AUTHZ-007 — Revalidate Mutation Authorization and Preconditions** *(Event-Driven)* — When an actor requests creation, edit, approval, rejection, or deactivation, the system shall revalidate identity, active status, role, `canResolveRequests` for Approver resolution actions, ownership, request state, and applicable business preconditions immediately before the mutation.
...
- **AUD-004 — Record Security-Relevant Failures** *(Event-Driven)* — When authentication fails, an expired session is used, authorization is denied, an inactive or capability-disabled Approver attempts protected approval access or resolution, an invalid transition is attempted, or suspected unauthorized resource access occurs, the system shall create a structured security event suitable for monitoring and alerting.
...
- **CFG-001 — Configure the Timeout Limit** *(Ubiquitous)* — The system shall treat the Pending-resolution timeout `X` (in days) as a configurable system parameter that can change without altering the domain workflow. **required configuration** — No default value is provided in code; the configuration key `NovaLeave:PendingRequestTimeoutDays` (positive integer) must be set in all environments before startup (official value 14 per `docs/adr/DR-001-runtime-configuration-values.md`).
- **CFG-002 — Configure Session Lifetime** *(Ubiquitous)* — The system shall treat authenticated-session lifetime and expiration behavior as configurable, consistent with the constitutional security baseline. **required configuration** — No default value is provided in code; the configuration key `NovaLeave:SessionTimeoutMinutes` (positive integer) must be set in all environments before startup (official value 30 per `docs/adr/DR-001-runtime-configuration-values.md`).
...
- **Request State**: Exactly one of `Pending`, `Approved`, `Rejected`, `CancelledByTimeout`, or `CancelledByApprover`.
...
- The MVP supports full working days only; weekends are excluded and holidays are counted as working days; there is no holiday calendar or per-user time-zone behavior. The single business date is Costa Rica time.
...
- **PD-003 — Global Approver Authority**: Any active Approver with `canResolveRequests=true` may resolve any eligible request; no one may resolve their own request.
...
- **PD-007 — Full-Day Calendar Rules**: Inclusive calendar dates and whole working days; Saturdays and Sundays excluded; holidays counted as working days; no holiday calendar; the single system business date uses `America/Costa_Rica`; no per-user time zones; earliest start is the following Costa Rica calendar day; zero-working-day ranges are rejected.
...
- **OQ-001 — Owner cancellation of a Pending request** *(RESOLVED 2026-07-16 as OUT OF MVP SCOPE)*: **RESOLVED — User cancellation of a Pending request is OUT OF MVP SCOPE.** No User cancellation route, action, contract, requirement, transition, or task is part of the MVP. *(Reserved identifier: FR-008.)*
...
- **OQ-003 — Inactive User operations** *(RESOLVED per current Constitution §4.4)*: Inactive users cannot create requests, edit requests, or execute actions that change request state. This is now explicitly stated in current Constitution §4.4. Update requirements, edge cases, scenarios, rules, and acceptance criteria accordingly.
  
- **OQ-004 — Mandatory reason for deactivation** *(RESOLVED as OUT OF MVP SCOPE)*: Whether Approver deactivation of an approved request requires a human-entered reason is undefined. **Out of MVP Scope**: The MVP does not introduce a mandatory deactivation-reason requirement until an approved specification exists. Do not invent new requirements.

- **OQ-005 — Calendar scope** *(RESOLVED per current Constitution and 2026-07-30 route decision)*: User calendar displays only the authenticated User's own authorized requests at `/calendario`. Approver calendar displays only the approved anonymized scope defined for eligible Approvers at `/calendario`. HR calendar displays all vacation requests across the organization, including all departments, only at `/rrhh/calendario`. Update FR-015, FR-017, AC-057, views, navigation, accessibility, permissions, and UI components accordingly. Do not broaden User or Approver permissions.
...
## Constitution Check
This specification has been checked against `.specify/memory/constitution.md` v6.0.1.
...
2. **Removal of per-user time zones** — v4.0.0 requires one system business date. The 2026-08-05 clarification fixes that date to Costa Rica time while retaining UTC storage. BR-003 now complies.
3. **Owner cancellation** — **RESOLVED — User cancellation of a Pending request is OUT OF MVP SCOPE.** No User cancellation route, action, contract, requirement, transition, or task is part of this MVP.
...
| Lifecycle invariants | Compliant under v6.0.1. The revised lifecycle (`CancelledByTimeout`, `CancelledByApprover`, pre-start deactivation, Pending editing) matches amended invariants 3, 4, 6–10, and 12; HR inactive status and Approver capability revalidation added. |
...
| Date and overlap rules | Compliant under v7.0.0 (invariants 3 and 4). Inclusive whole working days; weekends excluded; holidays counted; next-day minimum; zero-day rejection; overlap and adjacency defined; single Costa Rica business date and no per-user time zones. |
...
| Scope discipline | Compliant. Out-of-scope items are enumerated; only approved values are configurable; HR role and workflows are IN scope per current Constitution. |
**Constitution Result**: PASS — the specification aligns with Constitution v7.0.0; the two prior lifecycle conflicts are resolved by the v4.0.0 amendment; the HR role addition is resolved by the v6.0.0 amendment; Approver capability eligibility is clarified by the v6.0.1 amendment; no unresolved Constitution conflict remains.
**Planning Result**: READY FOR PLANNING — the core create-resolve-reserve-accrue workflow with HR read-only and capability management is fully specified and Constitution-aligned. Implementation is `NOT STARTED`; tests are `PLANNED, NOT IMPLEMENTED`; security controls are `PLANNED, NOT VERIFIED`; executable validation is `NOT VERIFIABLE` until source code and executable checks exist.
