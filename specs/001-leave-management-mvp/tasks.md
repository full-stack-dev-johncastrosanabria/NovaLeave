...
- [ ] T001 Create solution file `NovaLeave.sln` and planned project directories `src/NovaLeave.Domain/`, `src/NovaLeave.Application/`, `src/NovaLeave.Infrastructure/`, `src/NovaLeave.Presentation.Web/`, `tests/NovaLeave.UnitTests/`, `tests/NovaLeave.IntegrationTests/`, and `tests/NovaLeave.EndToEndTests/`
...
- [ ] T005 [P] Configure MVC and Razor Views package references in `src/NovaLeave.Presentation.Web/NovaLeave.Presentation.Web.csproj`
- [ ] T006 [P] Configure ASP.NET Core Identity UI package references in `src/NovaLeave.Presentation.Web/NovaLeave.Presentation.Web.csproj`
- [ ] T007 [P] Configure FluentValidation package references in `src/NovaLeave.Application/NovaLeave.Application.csproj` and `src/NovaLeave.Presentation.Web/NovaLeave.Presentation.Web.csproj`
...
- [ ] T048 Register Serilog request logging and structured logging enrichment in `src/NovaLeave.Presentation.Web/Program.cs`
- [ ] T049 Register built-in .NET `TimeProvider` directly in `src/NovaLeave.Presentation.Web/Program.cs`
- [ ] T050 Register `NovaLeaveOptions` validation with startup failure for missing/invalid values in `src/NovaLeave.Presentation.Web/Program.cs`
- [ ] T051 Create MVC filters for antiforgery, deny-by-default authorization, overposting prevention, ModelState validation feedback, safe error pages, and 403/404/409 responses in `src/NovaLeave.Presentation.Web/Filters/`
- [ ] T052 Create shared Razor layout shell in `src/NovaLeave.Presentation.Web/Views/Shared/_Layout.cshtml`
- [ ] T053 Create shared status badge partial in `src/NovaLeave.Presentation.Web/Views/Shared/_StatusBadge.cshtml`
- [ ] T054 Create shared toast partial in `src/NovaLeave.Presentation.Web/Views/Shared/_Toast.cshtml`
- [ ] T055 Create shared validation summary partial in `src/NovaLeave.Presentation.Web/Views/Shared/_ValidationSummary.cshtml`
- [ ] T056 Create shared calendar partial shell in `src/NovaLeave.Presentation.Web/Views/Shared/_Calendar.cshtml`
...
- [ ] T074 [US1] Implement User MVC controllers for `/mis-solicitudes`, `/mis-solicitudes/crear`, `/mis-solicitudes/{id}`, `/saldo`, and `/calendario` in `src/NovaLeave.Presentation.Web/Controllers/User/`
- [ ] T075 [US1] Implement dedicated User ViewModels in `src/NovaLeave.Presentation.Web/Models/User/`
- [ ] T076 [US1] Implement User Razor views with approved Spanish labels in `src/NovaLeave.Presentation.Web/Views/User/`
- [ ] T077 [US1] Wire User navigation and role context switcher in `src/NovaLeave.Presentation.Web/Views/Shared/_Layout.cshtml`
...
- [ ] T106 [US4] Add Approver POST route `/aprobaciones/{id}/desactivar` in `src/NovaLeave.Presentation.Web/Controllers/Approver/ApprovalsController.cs`
- [ ] T107 [US4] Add deactivation confirmation UI and denial messages in `src/NovaLeave.Presentation.Web/Views/Approver/Detail.cshtml`
...
- [ ] T143 [US8] Implement HR Approver capability MVC controller routes `/rrhh/aprobadores` and `/rrhh/aprobadores/{id}/capacidad` in `src/NovaLeave.Presentation.Web/Controllers/HR/ApproverCapabilitiesController.cs`
- [ ] T144 [US8] Implement capability management ViewModels in `src/NovaLeave.Presentation.Web/Models/HR/ApproverCapabilities/`
- [ ] T145 [US8] Implement capability list, modal confirmation, validation, concurrency, and toast UI in `src/NovaLeave.Presentation.Web/Views/HR/ApproverCapabilities/`
