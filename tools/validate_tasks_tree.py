...
Checks (all must pass):
  1. No unexpanded template artifacts ("$(@{", "System.Object[]", "{{") and no
     UTF-8 BOM in any file under tasks/ or docs/decisions/.
...
tree (see docs/decisions/DR-002-generated-artifact-review-rule.md).
"""
import codecs
import glob
import re
...
CITATION_ALLOWED = ("docs/decisions/DR-002-generated-artifact-review-rule.md",)

def main() -> int:
    fails = []

    # 1) artifacts + BOM
    for path in glob.glob("tasks/**/*.md", recursive=True) + glob.glob("docs/decisions/*.md"):
    ...
    canon = dict(re.findall(r"^- \[ \] (T\d{3}) (.+?)\r?$",
                            open(CANON, encoding="utf-8").read(), re.M))
    if len(canon) != 153:
    ...
    active = set(re.findall(r"\*\*(FR-\d+)\s+—", spec))
    covered = set()
    for path in task_files:
    ...
    if fails:
        ...
        for f in fails:
            print("  -", f)
        return 1
    print(f"PASS: {len(task_files)} TASK files round-trip against {len(canon)} canonical tasks; "
