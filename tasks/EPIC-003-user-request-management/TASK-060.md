...
Deliver the outcome defined by canonical task T060: [P] [US1] Add UC-02 role context switching integration tests in `tests/NovaLeave.IntegrationTests/UseCases/UC02SwitchRoleContextTests.cs`.

## Description

This task is a derived execution view of T060 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T060 executable without changing the approved scope. Original canonical text: [P] [US1] Add UC-02 role context switching integration tests in `tests/NovaLeave.IntegrationTests/UseCases/UC02SwitchRoleContextTests.cs`.
...
- Preserve the original canonical task identifier T060.
...
- [ ] The deliverable described by T060 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T060 and the listed requirements/use cases is preserved.
...
- Original task: T060 from specs/001-leave-management-mvp/tasks.md.
