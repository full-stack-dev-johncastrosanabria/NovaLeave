...
Deliver the outcome defined by canonical task T068: [P] [US1] Implement User request query use cases in `src/NovaLeave.Application/Requests/Queries/`.

## Description

This task is a derived execution view of T068 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T068 executable without changing the approved scope. Original canonical text: [P] [US1] Implement User request query use cases in `src/NovaLeave.Application/Requests/Queries/`.
...
- Preserve the original canonical task identifier T068.
...
- [ ] The deliverable described by T068 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T068 and the listed requirements/use cases is preserved.
...
- Original task: T068 from specs/001-leave-management-mvp/tasks.md.
