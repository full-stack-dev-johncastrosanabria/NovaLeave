...
Deliver the outcome defined by canonical task T077: [US1] Wire User navigation and role context switcher in `src/NovaLeave.Presentation.Web/Views/Shared/_Layout.cshtml`.

## Description

This task is a derived execution view of T077 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T077 executable without changing the approved scope. Original canonical text: [US1] Wire User navigation and role context switcher in `src/NovaLeave.Presentation.Web/Views/Shared/_Layout.cshtml`.
...
- Preserve the original canonical task identifier T077.
...
- [ ] The deliverable described by T077 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T077 and the listed requirements/use cases is preserved.
...
- Original task: T077 from specs/001-leave-management-mvp/tasks.md.
