...
Deliver the outcome defined by canonical task T131: [P] [US7] Implement HR balance and movement read queries in `src/NovaLeave.Application/HR/Balances/`.

## Description

This task is a derived execution view of T131 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T131 executable without changing the approved scope. Original canonical text: [P] [US7] Implement HR balance and movement read queries in `src/NovaLeave.Application/HR/Balances/`.
...
- Preserve the original canonical task identifier T131.
...
- [ ] The deliverable described by T131 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T131 and the listed requirements/use cases is preserved.
...
- Original task: T131 from specs/001-leave-management-mvp/tasks.md.
