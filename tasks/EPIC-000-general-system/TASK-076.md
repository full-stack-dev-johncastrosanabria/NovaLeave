...
Deliver the outcome defined by canonical task T076: [US1] Implement User Razor views with approved Spanish labels in `src/NovaLeave.Presentation.Web/Views/User/`.

## Description

This task is a derived execution view of T076 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T076 executable without changing the approved scope. Original canonical text: [US1] Implement User Razor views with approved Spanish labels in `src/NovaLeave.Presentation.Web/Views/User/`.
...
- Preserve the original canonical task identifier T076.
...
- [ ] The deliverable described by T076 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T076 and the listed requirements/use cases is preserved.
...
- Original task: T076 from specs/001-leave-management-mvp/tasks.md.
