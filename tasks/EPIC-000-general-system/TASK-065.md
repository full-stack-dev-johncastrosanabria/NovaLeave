...
Deliver the outcome defined by canonical task T065: [P] [US1] Add UC-07 balance and movement history tests in `tests/NovaLeave.IntegrationTests/UseCases/UC07ViewOwnBalanceTests.cs`.

## Description

This task is a derived execution view of T065 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T065 executable without changing the approved scope. Original canonical text: [P] [US1] Add UC-07 balance and movement history tests in `tests/NovaLeave.IntegrationTests/UseCases/UC07ViewOwnBalanceTests.cs`.
...
- Preserve the original canonical task identifier T065.
...
- [ ] The deliverable described by T065 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T065 and the listed requirements/use cases is preserved.
...
- Original task: T065 from specs/001-leave-management-mvp/tasks.md.
