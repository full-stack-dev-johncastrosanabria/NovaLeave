...
Deliver the outcome defined by canonical task T047: Register EF Core SQL Server DbContext in `src/NovaLeave.Infrastructure/DependencyInjection.cs`.

## Description

This task is a derived execution view of T047 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T047 executable without changing the approved scope. Original canonical text: Register EF Core SQL Server DbContext in `src/NovaLeave.Infrastructure/DependencyInjection.cs`.
...
- Preserve the original canonical task identifier T047.
...
- [ ] The deliverable described by T047 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T047 and the listed requirements/use cases is preserved.
...
- Original task: T047 from specs/001-leave-management-mvp/tasks.md.
