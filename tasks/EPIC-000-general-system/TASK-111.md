...
Deliver the outcome defined by canonical task T111: [P] [US5] Add catch-up accrual tests in `tests/NovaLeave.IntegrationTests/UseCases/UC17AccrualCatchUpTests.cs`.

## Description

This task is a derived execution view of T111 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T111 executable without changing the approved scope. Original canonical text: [P] [US5] Add catch-up accrual tests in `tests/NovaLeave.IntegrationTests/UseCases/UC17AccrualCatchUpTests.cs`.
...
- Preserve the original canonical task identifier T111.
...
- [ ] The deliverable described by T111 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T111 and the listed requirements/use cases is preserved.
...
- Original task: T111 from specs/001-leave-management-mvp/tasks.md.
