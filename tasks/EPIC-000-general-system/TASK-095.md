...
Deliver the outcome defined by canonical task T095: [P] [US3] Add timeout idempotency tests in `tests/NovaLeave.IntegrationTests/Idempotency/TimeoutIdempotencyTests.cs`.

## Description

This task is a derived execution view of T095 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T095 executable without changing the approved scope. Original canonical text: [P] [US3] Add timeout idempotency tests in `tests/NovaLeave.IntegrationTests/Idempotency/TimeoutIdempotencyTests.cs`.
...
- Preserve the original canonical task identifier T095.
...
- [ ] The deliverable described by T095 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T095 and the listed requirements/use cases is preserved.
...
- Original task: T095 from specs/001-leave-management-mvp/tasks.md.
