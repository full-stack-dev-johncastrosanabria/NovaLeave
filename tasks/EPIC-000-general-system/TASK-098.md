...
Deliver the outcome defined by canonical task T098: [US3] Implement system actor audit writer support in `src/NovaLeave.Application/Audit/SystemAuditWriter.cs`.

## Description

This task is a derived execution view of T098 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T098 executable without changing the approved scope. Original canonical text: [US3] Implement system actor audit writer support in `src/NovaLeave.Application/Audit/SystemAuditWriter.cs`.
...
- Preserve the original canonical task identifier T098.
...
- [ ] The deliverable described by T098 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T098 and the listed requirements/use cases is preserved.
...
- Original task: T098 from specs/001-leave-management-mvp/tasks.md.
