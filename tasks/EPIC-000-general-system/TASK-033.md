...
Deliver the outcome defined by canonical task T033: Create Application authorization facade abstraction in `src/NovaLeave.Application/Common/Interfaces/IAuthorizationServiceFacade.cs`.

## Description

This task is a derived execution view of T033 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T033 executable without changing the approved scope. Original canonical text: Create Application authorization facade abstraction in `src/NovaLeave.Application/Common/Interfaces/IAuthorizationServiceFacade.cs`.
...
- Preserve the original canonical task identifier T033.
...
- [ ] The deliverable described by T033 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T033 and the listed requirements/use cases is preserved.
...
- Original task: T033 from specs/001-leave-management-mvp/tasks.md.
