...
Deliver the outcome defined by canonical task T144: [US8] Implement capability management ViewModels in `src/NovaLeave.Presentation.Web/Models/HR/ApproverCapabilities/`.

## Description

This task is a derived execution view of T144 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T144 executable without changing the approved scope. Original canonical text: [US8] Implement capability management ViewModels in `src/NovaLeave.Presentation.Web/Models/HR/ApproverCapabilities/`.
...
- Preserve the original canonical task identifier T144.
...
- [ ] The deliverable described by T144 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T144 and the listed requirements/use cases is preserved.
...
- Original task: T144 from specs/001-leave-management-mvp/tasks.md.
