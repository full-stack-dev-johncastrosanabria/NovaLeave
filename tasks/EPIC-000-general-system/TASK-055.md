...
Deliver the outcome defined by canonical task T055: Create shared validation summary partial in `src/NovaLeave.Presentation.Web/Views/Shared/_ValidationSummary.cshtml`.

## Description

This task is a derived execution view of T055 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T055 executable without changing the approved scope. Original canonical text: Create shared validation summary partial in `src/NovaLeave.Presentation.Web/Views/Shared/_ValidationSummary.cshtml`.
...
- Preserve the original canonical task identifier T055.
...
- [ ] The deliverable described by T055 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T055 and the listed requirements/use cases is preserved.
...
- Original task: T055 from specs/001-leave-management-mvp/tasks.md.
