...
Deliver the outcome defined by canonical task T100: [US3] Register timeout job only when required configuration is present and valid in `src/NovaLeave.Infrastructure/DependencyInjection.cs`.

## Description

This task is a derived execution view of T100 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T100 executable without changing the approved scope. Original canonical text: [US3] Register timeout job only when required configuration is present and valid in `src/NovaLeave.Infrastructure/DependencyInjection.cs`.
...
- Preserve the original canonical task identifier T100.
...
- [ ] The deliverable described by T100 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T100 and the listed requirements/use cases is preserved.
...
- Original task: T100 from specs/001-leave-management-mvp/tasks.md.
