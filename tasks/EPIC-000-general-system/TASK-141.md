...
Deliver the outcome defined by canonical task T141: [P] [US8] Implement Approver capability list query in `src/NovaLeave.Application/HR/ApproverCapabilities/ListApproverCapabilities/`.

## Description

This task is a derived execution view of T141 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T141 executable without changing the approved scope. Original canonical text: [P] [US8] Implement Approver capability list query in `src/NovaLeave.Application/HR/ApproverCapabilities/ListApproverCapabilities/`.
...
- Preserve the original canonical task identifier T141.
...
- [ ] The deliverable described by T141 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T141 and the listed requirements/use cases is preserved.
...
- Original task: T141 from specs/001-leave-management-mvp/tasks.md.
