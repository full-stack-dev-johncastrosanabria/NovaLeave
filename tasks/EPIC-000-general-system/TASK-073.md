...
Deliver the outcome defined by canonical task T073: [US1] Implement FluentValidation validators for User request commands in `src/NovaLeave.Application/Requests/Validation/`.

## Description

This task is a derived execution view of T073 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T073 executable without changing the approved scope. Original canonical text: [US1] Implement FluentValidation validators for User request commands in `src/NovaLeave.Application/Requests/Validation/`.
...
- Preserve the original canonical task identifier T073.
...
- [ ] The deliverable described by T073 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T073 and the listed requirements/use cases is preserved.
...
- Original task: T073 from specs/001-leave-management-mvp/tasks.md.
