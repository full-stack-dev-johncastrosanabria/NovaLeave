...
Deliver the outcome defined by canonical task T042: Create BalanceMovement EF Core mapping in `src/NovaLeave.Infrastructure/Persistence/Configurations/BalanceMovementConfiguration.cs`.

## Description

This task is a derived execution view of T042 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T042 executable without changing the approved scope. Original canonical text: Create BalanceMovement EF Core mapping in `src/NovaLeave.Infrastructure/Persistence/Configurations/BalanceMovementConfiguration.cs`.
...
- Preserve the original canonical task identifier T042.
...
- [ ] The deliverable described by T042 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T042 and the listed requirements/use cases is preserved.
...
- Original task: T042 from specs/001-leave-management-mvp/tasks.md.
