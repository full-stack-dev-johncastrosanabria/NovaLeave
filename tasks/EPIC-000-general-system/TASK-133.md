...
Deliver the outcome defined by canonical task T133: [US7] Implement HR authorization policies in `src/NovaLeave.Application/Authorization/HRPolicies.cs`.

## Description

This task is a derived execution view of T133 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T133 executable without changing the approved scope. Original canonical text: [US7] Implement HR authorization policies in `src/NovaLeave.Application/Authorization/HRPolicies.cs`.
...
- Preserve the original canonical task identifier T133.
...
- [ ] The deliverable described by T133 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T133 and the listed requirements/use cases is preserved.
...
- Original task: T133 from specs/001-leave-management-mvp/tasks.md.
