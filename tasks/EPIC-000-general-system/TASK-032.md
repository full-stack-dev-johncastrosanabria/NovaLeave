...
Deliver the outcome defined by canonical task T032: Create Application DbContext abstraction in `src/NovaLeave.Application/Common/Interfaces/IApplicationDbContext.cs`.

## Description

This task is a derived execution view of T032 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T032 executable without changing the approved scope. Original canonical text: Create Application DbContext abstraction in `src/NovaLeave.Application/Common/Interfaces/IApplicationDbContext.cs`.
...
- Preserve the original canonical task identifier T032.
...
- [ ] The deliverable described by T032 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T032 and the listed requirements/use cases is preserved.
...
- Original task: T032 from specs/001-leave-management-mvp/tasks.md.
