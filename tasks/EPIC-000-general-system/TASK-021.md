...
Deliver the outcome defined by canonical task T021: Create movement type enum in `src/NovaLeave.Domain/Enums/MovementType.cs`.

## Description

This task is a derived execution view of T021 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T021 executable without changing the approved scope. Original canonical text: Create movement type enum in `src/NovaLeave.Domain/Enums/MovementType.cs`.
...
- Preserve the original canonical task identifier T021.
...
- [ ] The deliverable described by T021 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T021 and the listed requirements/use cases is preserved.
...
- Original task: T021 from specs/001-leave-management-mvp/tasks.md.
