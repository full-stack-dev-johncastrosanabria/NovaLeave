...
Deliver the outcome defined by canonical task T127: [P] [US7] Add HR forbidden mutation tests for approve/reject/deactivate/balance/role operations in `tests/NovaLeave.IntegrationTests/Security/HRRestrictionTests.cs`.

## Description

This task is a derived execution view of T127 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T127 executable without changing the approved scope. Original canonical text: [P] [US7] Add HR forbidden mutation tests for approve/reject/deactivate/balance/role operations in `tests/NovaLeave.IntegrationTests/Security/HRRestrictionTests.cs`.
...
- Preserve the original canonical task identifier T127.
...
- [ ] The deliverable described by T127 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T127 and the listed requirements/use cases is preserved.
...
- Original task: T127 from specs/001-leave-management-mvp/tasks.md.
