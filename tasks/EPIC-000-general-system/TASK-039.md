...
Deliver the outcome defined by canonical task T039: Create EF Core DbContext shell in `src/NovaLeave.Infrastructure/Persistence/NovaLeaveDbContext.cs`.

## Description

This task is a derived execution view of T039 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T039 executable without changing the approved scope. Original canonical text: Create EF Core DbContext shell in `src/NovaLeave.Infrastructure/Persistence/NovaLeaveDbContext.cs`.
...
- Preserve the original canonical task identifier T039.
...
- [ ] The deliverable described by T039 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T039 and the listed requirements/use cases is preserved.
...
- Original task: T039 from specs/001-leave-management-mvp/tasks.md.
