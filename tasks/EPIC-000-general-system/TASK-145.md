...
Deliver the outcome defined by canonical task T145: [US8] Implement capability list, modal confirmation, validation, concurrency, and toast UI in `src/NovaLeave.Presentation.Web/Views/HR/ApproverCapabilities/`.

## Description

This task is a derived execution view of T145 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T145 executable without changing the approved scope. Original canonical text: [US8] Implement capability list, modal confirmation, validation, concurrency, and toast UI in `src/NovaLeave.Presentation.Web/Views/HR/ApproverCapabilities/`.
...
- Preserve the original canonical task identifier T145.
...
- [ ] The deliverable described by T145 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T145 and the listed requirements/use cases is preserved.
...
- Original task: T145 from specs/001-leave-management-mvp/tasks.md.
