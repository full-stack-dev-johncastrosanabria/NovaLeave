...
Deliver the outcome defined by canonical task T038: Create Infrastructure Identity user extension in `src/NovaLeave.Infrastructure/Identity/ApplicationUser.cs`.

## Description

This task is a derived execution view of T038 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T038 executable without changing the approved scope. Original canonical text: Create Infrastructure Identity user extension in `src/NovaLeave.Infrastructure/Identity/ApplicationUser.cs`.
...
- Preserve the original canonical task identifier T038.
...
- [ ] The deliverable described by T038 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T038 and the listed requirements/use cases is preserved.
...
- Original task: T038 from specs/001-leave-management-mvp/tasks.md.
