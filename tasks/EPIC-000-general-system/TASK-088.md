# TASK-088 - Implement approval command converting reservation to deduction atomically with canResolveRequests=true revalidation, BR-037 negative projected-balance rejection, and BR-038 stale/concurrent projected-balance revalidation
...
Deliver the outcome defined by canonical task T088: [US2] Implement approval command converting reservation to deduction atomically with `canResolveRequests=true` revalidation, BR-037 negative projected-balance rejection, and BR-038 stale/concurrent projected-balance revalidation in `src/NovaLeave.Application/Approvals/ApproveRequest/`.

## Description

This task is a derived execution view of T088 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T088 executable without changing the approved scope. Original canonical text: [US2] Implement approval command converting reservation to deduction atomically with canResolveRequests=true revalidation, BR-037 negative projected-balance rejection, and BR-038 stale/concurrent projected-balance revalidation in `src/NovaLeave.Application/Approvals/ApproveRequest/`.
...
- Preserve the original canonical task identifier T088.
- Deliver only: Implement approval command converting reservation to deduction atomically with canResolveRequests=true revalidation, BR-037 negative projected-balance rejection, and BR-038 stale/concurrent projected-balance revalidation.
...
- [ ] The deliverable described by T088 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T088 and the listed requirements/use cases is preserved.
...
- Original task: T088 from specs/001-leave-management-mvp/tasks.md.
