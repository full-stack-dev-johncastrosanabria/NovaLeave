...
Deliver the outcome defined by canonical task T034: Create Application audit writer abstraction in `src/NovaLeave.Application/Common/Interfaces/IAuditWriter.cs`.

## Description

This task is a derived execution view of T034 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T034 executable without changing the approved scope. Original canonical text: Create Application audit writer abstraction in `src/NovaLeave.Application/Common/Interfaces/IAuditWriter.cs`.
...
- Preserve the original canonical task identifier T034.
...
- [ ] The deliverable described by T034 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T034 and the listed requirements/use cases is preserved.
...
- Original task: T034 from specs/001-leave-management-mvp/tasks.md.
