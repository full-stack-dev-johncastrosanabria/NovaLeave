...
Deliver the outcome defined by canonical task T136: [US7] Implement HR read-only Razor views with no resolution or balance modification actions in `src/NovaLeave.Presentation.Web/Views/HR/`.

## Description

This task is a derived execution view of T136 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T136 executable without changing the approved scope. Original canonical text: [US7] Implement HR read-only Razor views with no resolution or balance modification actions in `src/NovaLeave.Presentation.Web/Views/HR/`.
...
- Preserve the original canonical task identifier T136.
...
- [ ] The deliverable described by T136 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T136 and the listed requirements/use cases is preserved.
...
- Original task: T136 from specs/001-leave-management-mvp/tasks.md.
