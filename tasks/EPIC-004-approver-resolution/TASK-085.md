...
Deliver the outcome defined by canonical task T085: [P] [US2] Add Approver MVC E2E smoke tests in `tests/NovaLeave.EndToEndTests/Approver/ApproverJourneySmokeTests.cs`.

## Description

This task is a derived execution view of T085 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T085 executable without changing the approved scope. Original canonical text: [P] [US2] Add Approver MVC E2E smoke tests in `tests/NovaLeave.EndToEndTests/Approver/ApproverJourneySmokeTests.cs`.
...
- Preserve the original canonical task identifier T085.
...
- [ ] The deliverable described by T085 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T085 and the listed requirements/use cases is preserved.
...
- Original task: T085 from specs/001-leave-management-mvp/tasks.md.
