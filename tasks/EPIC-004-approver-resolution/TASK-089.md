...
Deliver the outcome defined by canonical task T089: [US2] Implement rejection command releasing reservation atomically in `src/NovaLeave.Application/Approvals/RejectRequest/`.

## Description

This task is a derived execution view of T089 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T089 executable without changing the approved scope. Original canonical text: [US2] Implement rejection command releasing reservation atomically in `src/NovaLeave.Application/Approvals/RejectRequest/`.
...
- Preserve the original canonical task identifier T089.
...
- [ ] The deliverable described by T089 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T089 and the listed requirements/use cases is preserved.
...
- Original task: T089 from specs/001-leave-management-mvp/tasks.md.
