...
Deliver the outcome defined by canonical task T102: [P] [US4] Add post-start and partial-deactivation denial tests in `tests/NovaLeave.IntegrationTests/UseCases/UC13DeactivateDenialTests.cs`.

## Description

This task is a derived execution view of T102 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T102 executable without changing the approved scope. Original canonical text: [P] [US4] Add post-start and partial-deactivation denial tests in `tests/NovaLeave.IntegrationTests/UseCases/UC13DeactivateDenialTests.cs`.
...
- Preserve the original canonical task identifier T102.
...
- [ ] The deliverable described by T102 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T102 and the listed requirements/use cases is preserved.
...
- Original task: T102 from specs/001-leave-management-mvp/tasks.md.
