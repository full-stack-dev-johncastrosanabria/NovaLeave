...
Deliver the outcome defined by canonical task T044: Create ApplicationUser EF Core mapping in `src/NovaLeave.Infrastructure/Persistence/Configurations/ApplicationUserConfiguration.cs`.

## Description

This task is a derived execution view of T044 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T044 executable without changing the approved scope. Original canonical text: Create ApplicationUser EF Core mapping in `src/NovaLeave.Infrastructure/Persistence/Configurations/ApplicationUserConfiguration.cs`.
...
- Preserve the original canonical task identifier T044.
...
- [ ] The deliverable described by T044 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T044 and the listed requirements/use cases is preserved.
...
- Original task: T044 from specs/001-leave-management-mvp/tasks.md.
