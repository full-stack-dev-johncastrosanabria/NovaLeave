...
Deliver the outcome defined by canonical task T037: Create typed configuration model in `src/NovaLeave.Application/Configuration/NovaLeaveOptions.cs`.

## Description

This task is a derived execution view of T037 from specs/001-leave-management-mvp/tasks.md. Implement or produce only the work explicitly described by the canonical task and the approved source artifacts.

## Context

The task exists to make canonical backlog item T037 executable without changing the approved scope. Original canonical text: Create typed configuration model in `src/NovaLeave.Application/Configuration/NovaLeaveOptions.cs`.
...
- Preserve the original canonical task identifier T037.
...
- [ ] The deliverable described by T037 exists in the planned path or documented artifact.
- [ ] Required test or evidence demonstrates the deliverable matches the approved source artifacts.
- [ ] Traceability to T037 and the listed requirements/use cases is preserved.
...
- Original task: T037 from specs/001-leave-management-mvp/tasks.md.
